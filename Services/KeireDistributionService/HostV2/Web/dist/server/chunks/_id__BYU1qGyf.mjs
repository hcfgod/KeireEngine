...
import { c as requireSupabase, n as apiError, r as apiResponse, t as MarketplaceApiError } from "./api_CeIcXSPx.mjs";
import { n as marketplaceEnabled } from "./marketplace_C3fGUB9S.mjs";
//#region Source/pages/marketplace/v1/products/[id].ts
...
var GET = async (context) => {
	try {
		...
		const [productResult, versionsResult, mediaResult, reviewsResult] = await Promise.all([
			...
			supabase.from("marketplace_product_versions").select("id,version,state,install_kind,minimum_engine_version,maximum_engine_version,platforms,architectures,renderer_capabilities,managed_api_version,release_notes_markdown,published_at").eq("product_id", id).in("state", [
				"published",
				"withdrawn",
				"security_revoked"
...
//#endregion
//#region \0virtual:astro:page:Source/pages/marketplace/v1/products/[id]@_@ts

...
- Coral fork commit ffae617 is the private native/.NET hosting bridge, copyright Studio Cherno contributors, under the Apache License 2.0.
...
SDK archives include these license texts under `third-party/licenses`, including the doctest license for complete project attribution even though doctest headers are not redistributed. SDL headers, its static archive, and its official CMake package are redistributed under `third-party/SDL3`; the host shader compiler, nethost, Coral managed bridge, and RID-specific .NET runtime files are redistributed under `bin`. Assimp, zlib, Jolt, Recast/Detour, miniaudio, Coral.Native, and the nethost import/static library are redistributed only as link dependencies under `lib`. Their implementation headers and source trees are not redistributed because these dependencies do not cross Kéire's public API.

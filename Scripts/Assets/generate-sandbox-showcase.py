...
EXAMPLES = (
    Example(
        ...
        (
            ...
            Property(
                "Metallic", "Metallic", SCALAR, 0.0, 0.0, "Surface", 0.0, 1.0, 0.01
            ),
            Property(
                "Roughness", "Roughness", SCALAR, 0.38, 0.24, "Surface", 0.0, 1.0, 0.01
            ),
        ),
    ),
    Example(
        ...
        (
            ...
            Property(
                "Tiling", "Tiling", VECTOR2, [3.0, 3.0], [5.0, 5.0], "Coordinates"
            ),
            Property(
                "Roughness", "Roughness", SCALAR, 0.3, 0.18, "Surface", 0.0, 1.0, 0.01
            ),
        ),
    ),
    Example(
    ...
    Example(
        ...
        (
            ...
            Property(
                "Cutoff", "Cutoff", SCALAR, 0.48, 0.56, "Transparency", 0.0, 1.0, 0.01
            ),
            Property(
                "NoiseScale",
                "Noise Scale",
    ...
    Example(
        ...
        (
            ...
            Property(
                "Roughness", "Roughness", SCALAR, 0.28, 0.2, "Metal", 0.0, 1.0, 0.01
            ),
            Property(
                "Anisotropy", "Anisotropy", SCALAR, 0.82, 0.94, "Metal", -1.0, 1.0, 0.01
            ),
        ),
    ),
    Example(
        ...
        (
            ...
            Property(
                "Opacity", "Opacity", SCALAR, 0.32, 0.24, "Transparency", 0.0, 1.0, 0.01
            ),
            Property(
                "Refraction", "Refraction", SCALAR, 0.14, 0.22, "Glass", 0.0, 1.0, 0.01
            ),
        ),
        alpha_mode=2,
        double_sided=True,
    ...
    Example(
        ...
        (
            ...
            Property(
                "Roughness", "Roughness", SCALAR, 0.78, 0.68, "Surface", 0.0, 1.0, 0.01
            ),
        ),
    ),
    Example(
    ...
    Example(
        ...
        (
            ...
            Property(
                "Opacity", "Opacity", SCALAR, 0.52, 0.42, "Transparency", 0.0, 1.0, 0.01
            ),
        ),
        alpha_mode=2,
        double_sided=True,
    ...
    Example(
        ...
        (
            ...
            Property(
                "Opacity", "Opacity", SCALAR, 0.38, 0.48, "Transparency", 0.0, 1.0, 0.01
            ),
        ),
        alpha_mode=2,
        double_sided=True,
...
def pin(
    name: str, value_type: int, direction: int, value: Any | None = None
) -> tuple[str, int, int, Any]:
    return (
        name,
        value_type,
        direction,
        default_value(value_type) if value is None else value,
    )


def node_spec(
    kind: str, value_type: int
) -> tuple[int, str, str, int, Any, list[tuple[str, int, int, Any]]]:
    if kind == "parameter" or kind == "constant":
        ...
        return (
            ...
            [pin("Value", value_type, OUTPUT)],
        )
    if kind == "texture_sample":
        number, type_id, name = NODE_TYPES[kind]
        pins = [
            pin("Texture", TEXTURE2D, INPUT),
            pin("UV", VECTOR2, INPUT),
            pin("RGBA", COLOR, OUTPUT),
            pin("RGB", VECTOR3, OUTPUT),
            pin("R", SCALAR, OUTPUT),
            pin("G", SCALAR, OUTPUT),
            pin("B", SCALAR, OUTPUT),
            pin("A", SCALAR, OUTPUT),
    ...
    if kind == "uv_transform":
        ...
        return (
            ...
            [
                pin("UV", VECTOR2, INPUT),
                pin("Tiling", VECTOR2, INPUT, [1.0, 1.0]),
                pin("Offset", VECTOR2, INPUT),
                pin("UV", VECTOR2, OUTPUT),
            ],
        )
    if kind in {"add", "multiply"}:
        ...
        return (
            ...
            default_value(value_type),
            [
                pin("A", value_type, INPUT),
                pin("B", value_type, INPUT),
    ...
    if kind in {"one_minus", "clamp", "sine"}:
        ...
        return (
            ...
            value_type,
            default_value(value_type),
            [pin(input_name, value_type, INPUT), pin("Result", value_type, OUTPUT)],
        )
    if kind == "dot":
        number, type_id, name = NODE_TYPES[kind]
        return (
    ...
    if kind == "smooth_step":
        ...
        return (
            ...
            default_value(value_type),
            [
                pin("Edge Min", value_type, INPUT),
                pin(
                    "Edge Max",
                    value_type,
                    INPUT,
                    1.0 if value_type == SCALAR else default_value(value_type),
                ),
                pin("Value", value_type, INPUT),
                pin("Result", value_type, OUTPUT),
            ],
        )
    if kind == "fresnel":
        ...
        return (
            ...
            0.0,
            [
                pin("Normal", VECTOR3, INPUT, [0.0, 0.0, 1.0]),
                pin("Power", SCALAR, INPUT, 5.0),
                pin("F0", SCALAR, INPUT, 0.04),
                pin("Fresnel", SCALAR, OUTPUT),
            ],
        )
    ...
    if kind == "simple_noise":
        ...
        return (
            ...
            [
                pin("UV", VECTOR2, INPUT),
                pin("Scale", SCALAR, INPUT, 5.0),
                pin("Detail", SCALAR, INPUT, 0.5),
    ...
    if kind == "hue_shift":
        ...
        return (
            ...
            default_value(COLOR),
            [
                pin("Color", COLOR, INPUT, [1.0, 1.0, 1.0, 1.0]),
                pin("Shift", SCALAR, INPUT),
                pin("Color", COLOR, OUTPUT),
            ],
        )
    if kind == "checkerboard":
        ...
        return (
            ...
            [
                ...
                pin("Scale", VECTOR2, INPUT, [8.0, 8.0]),
                pin("Color", COLOR, OUTPUT),
            ],
        )
    if kind == "panner":
        ...
        return (
            ...
            [
                pin("UV", VECTOR2, INPUT),
                pin("Speed", VECTOR2, INPUT, [0.1, 0.0]),
                pin("Time", SCALAR, INPUT),
                pin("UV", VECTOR2, OUTPUT),
            ],
        )
    if kind == "linear_gradient":
        ...
        return (
            ...
            [
                pin("UV", VECTOR2, INPUT),
                pin("Direction", VECTOR2, INPUT, [1.0, 0.0]),
                pin("Offset", SCALAR, INPUT),
    ...
    if kind == "facing_ratio":
        ...
        return (
            ...
            [
                pin("Normal", VECTOR3, INPUT, [0.0, 0.0, 1.0]),
                pin("Power", SCALAR, INPUT, 1.0),
                pin("Ratio", SCALAR, OUTPUT),
            ],
        )
    ...
    if kind == "wave":
        ...
        return (
            ...
            [
                pin("UV", VECTOR2, INPUT),
                pin("Direction", VECTOR2, INPUT, [1.0, 0.0]),
                pin("Frequency", SCALAR, INPUT, 8.0),
                pin("Phase", SCALAR, INPUT),
                pin("Wave", SCALAR, OUTPUT),
            ],
        )
    if kind == "triplanar":
        number, type_id, name = NODE_TYPES[kind]
        pins = [
            pin("Texture", TEXTURE2D, INPUT),
            pin("Position", VECTOR3, INPUT),
            pin("Normal", VECTOR3, INPUT, [0.0, 0.0, 1.0]),
            pin("Scale", SCALAR, INPUT, 1.0),
            pin("Blend Sharpness", SCALAR, INPUT, 4.0),
            pin("RGBA", COLOR, OUTPUT),
            pin("RGB", VECTOR3, OUTPUT),
            pin("R", SCALAR, OUTPUT),
            pin("G", SCALAR, OUTPUT),
    ...
    if kind == "height_to_normal":
        ...
        return (
            ...
            default_value(VECTOR3),
            [
                pin("Height", SCALAR, INPUT, 0.5),
                pin("Strength", SCALAR, INPUT, 1.0),
                pin("Normal", VECTOR3, OUTPUT),
            ],
        )
...
def master_pins(output: int) -> list[tuple[str, int, int, Any]]:
    pins = [
        pin("BaseColor", COLOR, INPUT, [1.0, 1.0, 1.0, 1.0]),
        pin("Metallic", SCALAR, INPUT, 0.0),
        pin("Roughness", SCALAR, INPUT, 0.5),
        pin("Specular", SCALAR, INPUT, 0.5),
        pin("ClearCoat", SCALAR, INPUT, 0.0),
        pin("ClearCoatRoughness", SCALAR, INPUT, 0.25),
        pin("SheenColor", COLOR, INPUT, [0.0, 0.0, 0.0, 1.0]),
        pin("SheenRoughness", SCALAR, INPUT, 0.5),
        pin("Normal", VECTOR3, INPUT, [0.0, 0.0, 1.0]),
        pin("DetailNormal", VECTOR3, INPUT, [0.0, 0.0, 1.0]),
        pin("Parallax", SCALAR, INPUT, 0.0),
        pin("Emission", COLOR, INPUT, [0.0, 0.0, 0.0, 1.0]),
        pin("Occlusion", SCALAR, INPUT, 1.0),
        pin("Opacity", SCALAR, INPUT, 1.0),
        pin("SubsurfaceColor", COLOR, INPUT, [1.0, 0.35, 0.25, 1.0]),
        pin("Subsurface", SCALAR, INPUT, 0.0),
        pin("Anisotropy", SCALAR, INPUT, 0.0),
        pin("Tangent", VECTOR3, INPUT, [1.0, 0.0, 0.0]),
        pin("Transmission", SCALAR, INPUT, 0.0),
        pin("IndexOfRefraction", SCALAR, INPUT, 1.5),
        pin("Refraction", SCALAR, INPUT, 0.0),
        pin("Thickness", SCALAR, INPUT, 1.0),
        pin("MaterialAttributes", MATERIAL_ATTRIBUTES, INPUT),
        pin("WorldPositionOffset", VECTOR3, INPUT),
...
class GraphBuilder:
    def __init__(self, scope: str, output: int, material_output: bool = False) -> None:
        ...
        output_name = (
            ...
            if material_output
                ...
                "Unlit Shader Output"
                if output == UNLIT
                else "Transparent Shader Output"
                if output == TRANSPARENT
                else "Lit Shader Output"
            )
        )
        self.master = self._append_node(
            0,
            "keire.output.material",
            output_name,
            COLOR,
            [1.0, 1.0, 1.0, 1.0],
            master_pins(output),
    ...
    def _append_node(
        ...
        value_type: int,
        value: Any,
        pins: list[tuple[str, int, int, Any]],
        position: list[float],
        *,
        symbol: str = "",
        ...
        encoded_pins = []
        for pin_ordinal, (pin_name, pin_type, direction, pin_default) in enumerate(
            pins
        ):
            encoded_pins.append(
                {
                    "id": stable_id(
                        f"{node_id}/pin/{pin_ordinal}/{direction}/{pin_name}"
                    ),
                    "name": pin_name,
                    "type": pin_type,
                    "direction": direction,
        ...
        node = {
            ...
            or {"description": "", "category": "", "sortPriority": 0},
            "pins": encoded_pins,
        }
    ...
    def add(
        ...
        *,
        ...
        kind_number, type_id, default_name, canonical_type, canonical_value, pins = (
            node_spec(kind, value_type)
        )
        defaults = defaults or {}
        pins = [
            (pin_name, pin_type, direction, defaults.get(pin_name, pin_default))
        ...
        return self._append_node(
            ...
            canonical_type,
            canonical_value if value is None else value,
            pins,
    ...
    def parameter(
        self, prop: Property, position: tuple[float, float], prefix: str = ""
    ) -> dict[str, Any]:
    ...
    def _pin(node: dict[str, Any], name: str, direction: int) -> str:
        matches = [
            entry
            for entry in node["pins"]
            if entry["name"] == name and entry["direction"] == direction
        ]
        if len(matches) != 1:
            raise ValueError(
                f"Expected one {name} pin on {node['name']}, found {len(matches)}"
            )
    ...
    def connect(
        ...
        self.connections.append(
            {
                "id": stable_id(
                    f"{self.scope}/connection/{ordinal}/{source['id']}/{output_pin}/{target['id']}/{input_pin}"
                ),
...
def add_parameter_connections(
    builder: GraphBuilder, example: Example, prefix: str = ""
) -> dict[str, dict[str, Any]]:
    parameters: dict[str, dict[str, Any]] = {}
    for index, prop in enumerate(example.properties):
        parameters[prop.symbol] = builder.parameter(
            prop, (40.0, 60.0 + index * 140.0), prefix
        )
    return parameters
...
def author_network(
    ...
    if style == "paint":
        ...
        normalized = builder.add(
            "multiply", SCALAR, (x + 660.0, 280.0), defaults={"B": 0.5}
        )
        add_half = builder.add("add", SCALAR, (x + 860.0, 280.0), defaults={"B": 0.5})
        emission = builder.add(
            "lerp", COLOR, (x + 860.0, 60.0), defaults={"A": [0.0, 0.0, 0.0, 1.0]}
        )
        ...
        mask = builder.add(
            "smooth_step", SCALAR, (x + 500.0, 180.0), defaults={"Edge Max": 0.62}
        )
        ...
        tangent = builder.add(
            "world_normal", VECTOR3, (x + 240.0, 520.0), name="Brushing Direction"
        )
        ...
        noise = builder.add(
            "gradient_noise", SCALAR, (x + 320.0, 520.0), defaults={"Scale": 12.0}
        )
        detail = builder.add(
            "height_to_normal", VECTOR3, (x + 600.0, 500.0), defaults={"Strength": 2.0}
        )
        ...
        noise = builder.add(
            "gradient_noise", SCALAR, (x + 240.0, 260.0), defaults={"Scale": 8.0}
        )
        mask = builder.add(
            "smooth_step", SCALAR, (x + 500.0, 260.0), defaults={"Edge Max": 0.62}
        )
        builder.connect(uv, "UV", noise, "UV")
        ...
        panner = builder.add(
            "panner", VECTOR2, (x + 240.0, 260.0), defaults={"Speed": [0.0, 0.35]}
        )
        gradient = builder.add(
            "linear_gradient",
            SCALAR,
            (x + 500.0, 260.0),
        ...
        displacement = builder.add(
            "multiply", VECTOR3, (x + 760.0, 300.0), defaults={"A": [0.0, 1.0, 0.0]}
        )
...
def shader_graph(example: Example) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    builder = GraphBuilder(
        f"sandbox/shader/{example.number:02d}/{example.slug}", example.output
    )
...
def material_surface_graph(example: Example) -> dict[str, Any]:
    ...
    material_example = Example(
        ...
        material_properties,
        example.alpha_mode,
        example.double_sided,
    )
    builder = GraphBuilder(
        f"sandbox/material/{example.number:02d}/{example.slug}/surface",
        example.output,
        True,
    )
...
def material_graph(
    example: Example, shader_asset: str, shader_parameters: dict[str, dict[str, Any]]
) -> dict[str, Any]:
...
def graph_paths(example: Example) -> tuple[Path, Path]:
    ...
    shader = (
        ...
        / "ShaderGraphs"
        / level
        / f"SG_{example.number:02d}_{example.slug}.keireshadergraph"
    )
    material = (
        SHOWCASE_ROOT
        / "MaterialGraphs"
...
def text_metadata(path: Path) -> dict[str, Any]:
    return metadata(
        stable_id(f"sandbox/text/{path.relative_to(SANDBOX_ROOT).as_posix()}"),
        TEXT_TYPE,
        "Keire.Text",
        1,
    )
...
def transform(
    position: list[float], scale: list[float] | None = None
) -> dict[str, Any]:
    return {
        "type": TRANSFORM_TYPE,
        "version": 1,
...
def showcase_scene(
    material_ids: dict[int, str], plinth_material: str
) -> dict[str, Any]:
    ...
    entities.append(
        scene_entity(
            "material-lab",
            "Material Lab - 12 Shader and Material Graph Pairs",
            [0.0, 0.0, 0.0],
        )
    )
    entities.append(
        scene_entity("vfx-lab", "VFX Lab - Production Effects", [0.0, 0.0, 0.0])
    )
    entities.append(
        scene_entity(
            "camera",
    ...
    entities[-1]["components"][0]["data"]["rotation"] = [
        0.880651,
        0.113152,
        -0.376104,
        0.264946,
    ...
    for index, example in enumerate(EXAMPLES):
        ...
        entities.append(
            scene_entity(
                f"material/{example.number:02d}",
                f"{example.number:02d} - {example.display_name}",
                [x_positions[column], 1.15, z_positions[row]],
                [
                    mesh_renderer(
                        material_ids[example.number],
                        BUILTIN_MESHES[index % len(BUILTIN_MESHES)],
                    ),
                    orbit_component(),
                ],
                parent=level_roots[example.level],
    ...
    for index, (name, effect) in enumerate(VFX_EXAMPLES):
        entities.append(
            scene_entity(
                f"vfx/{index}",
                f"VFX {index + 1:02d} - {name}",
                [x_positions[index], 1.2, 10.0],
                [vfx_component(effect, 1000 + index * 7919)],
                parent=vfx_root,
            )
        )
    return {
...
def expected_files() -> dict[Path, bytes]:
    ...
    for example in EXAMPLES:
        ...
        shader_asset = stable_id(
            f"sandbox/shader-asset/{example.number:02d}/{example.slug}"
        )
        material_asset = stable_id(
            f"sandbox/material-asset/{example.number:02d}/{example.slug}"
        )
        ...
        material_subassets = [
            derive_subasset_id(
                material_asset, "material-graph/shader/14650fb0739d0383"
            ),
        ...
        result[Path(f"{shader_path}.keiremeta")] = json_text(
            metadata(
                shader_asset,
                SHADER_GRAPH_TYPE,
                "Keire.ShaderGraph",
                16,
                subassets=shader_subassets,
            )
        ).encode("utf-8")
        result[material_path] = json_text(material).encode("utf-8")
        result[Path(f"{material_path}.keiremeta")] = json_text(
            metadata(
                material_asset,
                MATERIAL_GRAPH_TYPE,
                "Keire.MaterialGraph",
                5,
                dependencies=[shader_asset],
                subassets=material_subassets,
            )
        ).encode("utf-8")

    material_readme = SHOWCASE_ROOT / "README.md"
    result[material_readme] = showcase_readme().encode("utf-8")
    result[Path(f"{material_readme}.keiremeta")] = json_text(
        text_metadata(material_readme)
    ).encode("utf-8")
    ...
    result[Path(f"{PLINTH_MATERIAL_PATH}.keiremeta")] = json_text(
        metadata(
            plinth_material,
            MATERIAL_TYPE,
            "Keire.Material",
            5,
            dependencies=[DEFAULT_SURFACE_SHADER],
        )
    ).encode("utf-8")
    result[SCENE_PATH] = json_text(
        showcase_scene(material_ids, plinth_material)
    ).encode("utf-8")
    result[Path(f"{SCENE_PATH}.keiremeta")] = json_text(
        metadata(
            stable_id("sandbox/scene-asset/showcase"), SCENE_TYPE, "Keire.Scene", 6
        )
    ).encode("utf-8")
    result[SCRIPT_PATH] = showcase_script().encode("utf-8")
    result[Path(f"{SCRIPT_PATH}.keiremeta")] = json_text(
        metadata(stable_id("sandbox/script/showcase-orbit"), TEXT_TYPE, "Keire.Text", 1)
    ).encode("utf-8")
    result[VFX_GUIDE_PATH] = vfx_guide().encode("utf-8")
    result[Path(f"{VFX_GUIDE_PATH}.keiremeta")] = json_text(
        text_metadata(VFX_GUIDE_PATH)
    ).encode("utf-8")
...
def managed_files() -> set[Path]:
    ...
    files: set[Path] = {
        SCENE_PATH,
        Path(f"{SCENE_PATH}.keiremeta"),
        SCRIPT_PATH,
        Path(f"{SCRIPT_PATH}.keiremeta"),
        VFX_GUIDE_PATH,
        Path(f"{VFX_GUIDE_PATH}.keiremeta"),
...
def check() -> list[str]:
    ...
    if LEGACY_GRAPH_ROOT.exists():
        errors.append(
            f"legacy graph root still exists: {LEGACY_GRAPH_ROOT.relative_to(REPOSITORY_ROOT)}"
        )
    ...
    for path, content in expected.items():
        if not path.is_file():
            errors.append(
                f"missing generated file: {path.relative_to(REPOSITORY_ROOT)}"
            )
        elif path.read_bytes() != content:
            errors.append(
                f"generated file differs: {path.relative_to(REPOSITORY_ROOT)}"
            )
...
def safe_remove_tree(path: Path) -> None:
    ...
    expected = {
        LEGACY_GRAPH_ROOT.resolve(),
        LEGACY_GENERATED_GRAPH_ROOT.resolve(),
        SHOWCASE_ROOT.resolve(),
    }
...
def generate() -> None:
    ...
    if errors:
        raise RuntimeError(
            "Sandbox showcase generation did not converge: " + "; ".join(errors)
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Validate generated content without changing files.",
    )
    ...
    if arguments.check:
        ...
        if errors:
            ...
            return 1
        print(
            f"Sandbox showcase is current ({len(EXAMPLES)} Shader Graph and Material Graph pairs)."
        )
        return 0
    generate()
    print(
        f"Generated Sandbox showcase ({len(EXAMPLES)} Shader Graph and Material Graph pairs)."
    )
    return 0

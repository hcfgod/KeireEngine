...
$WorkspaceLock = Enter-KeireWorkspaceLock -RepositoryRoot $Root -CommandName "package-hub-installer"
try {
...
Invoke-CheckedWindowsCommand {
    & (Join-Path $PSScriptRoot "package-hub.ps1") -Generator $Generator -Architecture $Architecture `
        -Toolset $Toolset -CI:$CI -Update:$Update -Generate:$Generate -AllowDirty:$AllowDirty -StageOnly
} "Standalone Hub distribution gate"

$distributionName = "$($Project.ARTIFACT_PREFIX)-hub-windows-$Architecture-Dist"
$distribution = Join-Path $Root "Build\Distributions\$distributionName"
Assert-WindowsHubPackageStage $distribution $Project.HUB_TARGET $Project.CLIENT_TARGET $Project.PROJECT_NAMESPACE
...
if (-not $makensisPath) {
    throw "NSIS 3 was not found. Install it with 'winget install NSIS.NSIS', then rerun package-hub-installer."
}
...
$artifactBase = "$($Project.ARTIFACT_PREFIX)-hub-windows-$Architecture-$($Project.PROJECT_VERSION)-setup"
...
if (-not (Test-Path -LiteralPath $artifact -PathType Leaf)) {
    throw "NSIS did not produce the expected standalone Hub installer: $artifact"
}
...
Write-Host "==> Windows standalone Hub installer created: $artifact"

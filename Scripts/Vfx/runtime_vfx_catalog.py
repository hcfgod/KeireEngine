...
NODE_CLASSES = {"Operator", "Parameter", "Constant", "Attribute", "Subgraph", "Block", "Context", "Output"}
...
def load_runtime_catalog(path: Path = DEFAULT_CONTRACT) -> list[RuntimeVfxNode]:
    ...
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if not match:
            raise ValueError(f"{path}:{line_number}: invalid VFX catalog contract entry")
        ...
        if entry.node_class not in NODE_CLASSES:
            raise ValueError(f"{path}:{line_number}: invalid node class {entry.node_class!r}")
        if entry.support not in SUPPORT_TIERS:
            raise ValueError(f"{path}:{line_number}: invalid support tier {entry.support!r}")
        if entry.backend not in BACKEND_TIERS:
            raise ValueError(f"{path}:{line_number}: invalid backend tier {entry.backend!r}")

...
assert_true grep -F -q 'cache_key="${coral_commit:0:12}-$variant_key"' "$ROOT/Scripts/Unix/coral.sh"
assert_false grep -F -q 'Patches/Coral' "$ROOT/Scripts/Unix/coral.sh"
assert_false grep -F -q 'git apply' "$ROOT/Scripts/Unix/coral.sh"
assert_true grep -F -q 'git clone --quiet --no-hardlinks --no-checkout "$source_path"' \
  "$ROOT/Scripts/Unix/coral.sh"
...
assert_true grep -F -q 'CORAL_URL=https://github.com/hcfgod/Coral.git' "$ROOT/Config/Dependencies.lock"
assert_true grep -F -q 'CORAL_COMMIT=ffae6176ec5d922f61e3b5f944a133837715a706' \
  "$ROOT/Config/Dependencies.lock"
assert_false test -e "$ROOT/Patches/Coral"
...
assert_true test -f "$ROOT/KeireCore/Shaders/BuiltinDeferredGBuffer.hlsl"
assert_true test -f "$ROOT/KeireCore/Shaders/BuiltinDeferredLighting.hlsl"
assert_true test -f "$ROOT/KeireCore/Shaders/BuiltinIrradyn.hlsl"
assert_true grep -R -q 'BuiltinUnlitShaders.h' "$ROOT/KeireCore/Source/Rendering"
assert_true grep -q 'BuiltinDeferredGBuffer' "$ROOT/Scripts/Unix/builtin-shaders.sh"
assert_true grep -q 'BuiltinDeferredLighting' "$ROOT/Scripts/Windows/builtin-shaders.ps1"
assert_true grep -q 'BuiltinIrradyn' "$ROOT/Scripts/Unix/builtin-shaders.sh"
assert_true grep -F -q 'BuiltinLighting.hlsli' "$ROOT/Scripts/Unix/builtin-shaders.sh"
assert_true grep -F -q '"${source_files[@]}" "${include_files[@]}"' "$ROOT/Scripts/Unix/builtin-shaders.sh"
assert_true grep -F -q -- '-I "$ROOT/KeireCore/Shaders"' "$ROOT/Scripts/Unix/builtin-shaders.sh"
...
for path in bin/CoreAssetTool bin/CoreAssetWorker lib/libCoreZstd.a include/Core/Math/Math.h include/Core/ECS/Component.h include/Core/ECS/Entity.h include/Core/ECS/Components/TransformComponent.h include/Core/ECS/Components/DirectionalLightComponent.h include/Core/ECS/Components/AudioComponents.h include/Core/ECS/Components/UiDocumentComponent.h include/Core/Ui/UiToolkit.h include/Core/Assets/Asset.h include/Core/Assets/AssetSystem.h include/Core/Assets/AssetPipeline.h include/Core/Assets/InputActionAsset.h include/Core/Input/Input.h samples/KeireSandbox/Assets/Input/DefaultInput.keireinput.keiremeta third-party/licenses/zstandard-LICENSE.txt third-party/licenses/entt-LICENSE.txt third-party/licenses/glm-COPYING.txt; do
  mkdir -p "$package_stage/$(dirname "$path")"; : > "$package_stage/$path"
done

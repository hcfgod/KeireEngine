...
project_generation_fingerprint() {
    local root="$1" source_root
    {
        for source_root in "$CORE_DIRECTORY" "$CLIENT_DIRECTORY" "$HUB_DIRECTORY" "$TESTS_DIRECTORY" \
          ...
          KeireRenderTests KeireRuntime KeireManaged KeireManaged.Tests KeireEditorManaged \
          KeireEditorManaged.Tests KeireManaged.Generators KeireManaged.Generators.Tests \
          SourceModules Scripts/Premake; do
...
validate_package_stage() {
    ...
    asset_required+=("lib/libharfbuzz.a" "lib/libfreetype.a" "lib/libfribidi.a" "lib/libunibreak.a"

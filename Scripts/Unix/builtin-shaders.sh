...
source_file="$ROOT/KeireCore/Shaders/BuiltinUnlit.hlsl"
...
if [[ -f "$generated" && "$generated" -nt "$source_file" && "$generated" -nt "$compiler" ]]; then
  exit 0
fi
...
for index in "${!names[@]}"; do
  "$compiler" "$source_file" -s HLSL -d "${destinations[$index]}" -t "${stages[$index]}" \
    -e "${entries[$index]}" -o "$temporary/${files[$index]}"
done

{
  printf '#pragma once\n\nnamespace Keire::Detail\n{\n'
  for index in "${!names[@]}"; do
    path="$temporary/${files[$index]}"
    printf '    inline constexpr unsigned char BuiltinUnlit%s[] = {\n' "${names[$index]}"
    od -An -v -t x1 "$path" | awk '{ printf "        "; for (i = 1; i <= NF; ++i) printf "0x%s, ", $i; printf "\n" }'
    printf '    };\n'
    printf '    inline constexpr unsigned int BuiltinUnlit%sSize = %s;\n' "${names[$index]}" "$(wc -c < "$path" | tr -d ' ')"
  done

...
namespace
{
    class TestPropertyEditor : public KeireEditor::IPropertyEditor
    {
      public:
        ...
        bool EditAsset(std::string_view, Keire::AssetId& value, std::optional<Keire::AssetTypeId> expectedType,
                       const std::string_view expectedManagedType = {}) override
        {
            ...
            ExpectedManagedType = expectedManagedType;
        ...
        std::string ExpectedManagedType;
...
TEST_CASE("ScriptableObject property drawers preserve the declared managed type constraint")
{
    KeireEditor::PropertyDrawerRegistry drawers;
    TestPropertyEditor editor;
    Keire::ComponentProperty property{"settings", "Settings", {}, Keire::ComponentPropertyKind::Asset};
    property.ExpectedAssetType = Keire::ManagedDataAsset::StaticType();
    property.ReferenceKind = Keire::ManagedReferenceKind::ScriptableObject;
    property.DeclaredManagedType = "Tests.PlayerSettings";
    Keire::ComponentPropertyValue value = Keire::AssetId{};

    CHECK(drawers.Draw(editor, CustomComponent::StaticType(), property, value));
    CHECK(editor.ExpectedAssetType == Keire::ManagedDataAsset::StaticType());
    CHECK(editor.ExpectedManagedType == "Tests.PlayerSettings");
}
...
TEST_CASE("GPU occlusion diagnostics expose readback age accounting and fallback without hiding direct draws")
{
    ...
    CHECK(presentation.Recording == "Occlusion recording 0.1 ms depth / 0.2 ms pyramid / 0.3 ms cull");
...
TEST_CASE("per-surface GPU occlusion diagnostics preserve typed fallback and source-frame context")
{
    ...
    CHECK(presentation.Pyramid.find("frame aggregate: 8 dispatches / 3 indirect draws") != std::string::npos);

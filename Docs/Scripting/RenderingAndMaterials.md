...
DirectionalLight? sun = GetComponent<DirectionalLight>();
PointLight? lamp = GetComponent<PointLight>();
SpotLight? flashlight = GetComponent<SpotLight>();
```
...
`Camera` exposes projection and clear modes, primary selection, priority, perspective field of view,
...
private Material? _detailMaterial;
...
`Vector3`, `Vector4`, `Color`, and direct `Texture` objects. Each renderer supports at most 64 overrides, and each
...
```csharp
DynamicMaterial visor = GetComponent<MeshRenderer>()!.GetMaterialInstance(1);
...
private MaterialParameterCollection? _weather;

private MaterialParameterCollectionInstance? _weatherRuntime;
...
4. slot-specific `DynamicMaterial` value.
...
```csharp
SpotLight? light = GetComponent<SpotLight>();
...
gameplay contract. Scripts can hold direct `ShaderGraph`, `ShaderGraphInstance`, `MaterialGraph`, and
`MaterialInstance` references and use `Assets.LoadRuntime` to observe

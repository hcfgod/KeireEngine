...
| Runtime UI | `UIDocument`, retained `VisualElement` trees, controls, event propagation, binding, custom-element registration, virtualization, panel scaling, and world/screen targets | UI Toolkit replaces Canvas authoring; custom font rasterization, advanced shaping, image-atlas refinement, localization, and platform accessibility adapters remain follow-up work |
...
| Runtime UI | **Partial** | UI Toolkit documents and styles; generation-safe live `UIDocument` queries; managed visual trees, controls, focus/pointer capture, event propagation, data binding, custom elements, virtualized collections, bounded transitions/gradients, and screen/camera/render-texture/world presentation | Localization, advanced font shaping, and platform screen-reader adapters |

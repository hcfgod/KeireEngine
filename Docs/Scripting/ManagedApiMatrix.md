...
| Inspector serialization | **Production** | Stable fields; v4 custom atomic values and migrations; serialization callbacks; nested/reference graphs; retained property drawers, decorators, custom editors, mixed multi-edit values, and atomic SerializedObject writes | Native Inspector region rendering for every specialized retained control continues to expand |
| Managed Editor SDK | **Partial** | Exact allowlist catalog; generation lifecycle and quarantine; retained inspectors; scripted importer/artifact contracts; windows, tools, menus, settings, selection, Undo, preferences, Asset Database and transactional build pipeline | Complete native retained-tree presentation, hard-process importer execution, layout restoration, safe-mode UI, and package-stage build invocation |
| Runtime services | **Production** | Application-owned discovery, deterministic dependency order, optional quarantine, lifetime tokens, frame updates, reverse stop, and bounded hot-reload state transfer | Service-specific editor diagnostics UI |
| Native source-module bindings | **Production** | Stable service/method descriptors, bounded ABI validation, exact candidate matching, generation/thread checks, and incremental typed-stub generation | Additional explicitly bounded value families as real module contracts require them |

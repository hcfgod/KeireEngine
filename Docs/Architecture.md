...
complete-array transactions. Per-renderer material property blocks are bounded, validated transient component state:
...
boundary. A standalone `Single` transition loads and validates a replacement while the current `SceneRuntimeSession`
continues to run, starts the replacement physics/audio/VFX/UI/script worlds, verifies its active camera, and only then
publishes it and stops the previous session. Activation failure retains the previous world and a bounded operation
diagnostic. Destination lifecycle callbacks receive a callback-local active-scene view without publishing that candidate
to the surrounding runtime before the transaction commits. Additive loading is rejected until cross-scene subsystem
ownership and unload can be one explicit
transaction. Runtime render-environment replacement validates the complete value before publication. Packaged-player
state is process-transient; Editor Play stores a separate Play-session override and never mutates the project settings
document.

...
TEST_CASE("Player support release catalogs derive HTTPS URLs and load bounded packages")
{
...
TEST_CASE("Player support release catalogs reject identity traversal corruption and version mismatches")
{
    ...
    Keire::Detail::WriteTextFileAtomically(
        path,
        R"({"schemaVersion":1,"engineVersion":"1.2.3","packages":[{"id":"test","platform":"linux","architecture":"arm64","file":"../test.keireplayersupport","size":1,"sha256":"not-a-hash"}]})");
    CHECK_THROWS_AS(

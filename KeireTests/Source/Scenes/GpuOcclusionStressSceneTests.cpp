...
TEST_CASE("Sandbox GPU occlusion stress scene stays above Automatic activation thresholds")
{
    ...
    CHECK(occluderData.at("material").get<std::string>() == StudioPaintMaterial);
    ...
    for (const auto& object : scene.Objects)
    {
        ...
        CHECK(data.at("material").get<std::string>() == StudioPaintMaterial);
...
TEST_CASE("Sandbox GPU occlusion stress scene references an opaque occlusion-compatible Shader Graph material")
{
    ...
    REQUIRE(sceneMetadata.at("dependencies").size() == 1U);
    CHECK(sceneMetadata.at("dependencies").front().get<std::string>() == StudioPaintMaterial);
    ...
    const auto materialMetadata =
        nlohmann::json::parse(KeireTests::ReadFile(std::filesystem::current_path() / StudioPaintMaterialMetadataPath));
    CHECK(std::ranges::any_of(materialMetadata.at("subAssets"), [](const nlohmann::json& value)
                              { return value.get<std::string>() == StudioPaintMaterial; }));

    const auto materialGraph =
        nlohmann::json::parse(KeireTests::ReadFile(std::filesystem::current_path() / StudioPaintMaterialGraphPath));
    ...
    const auto shaderSource = KeireTests::ReadFile(std::filesystem::current_path() / StudioPaintShaderGraphPath);

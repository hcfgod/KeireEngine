...
- Restored hosted CI parity with local launchers: quality checks use the pinned Clang 22 formatter, Windows shader
  ...
  JSON dependency, and macOS dependency builds avoid Xcode 16.4's obsolete zlib platform macro. Windows fast checks
  fall back to `git grep` when `rg` is unavailable, and the strict GCC/Clang/TSan/coverage warnings exposed by the
  hosted matrix are clean. Cleared the clang-tidy, ShellCheck, and pinned Ruff blockers exposed after the earlier
  failures were fixed.

...
namespace KeireEditor
{
    namespace
    {
        ...
        [[nodiscard]] bool HasEquivalentRuntimeShaders(const Keire::ShaderGraphDefinition& previousDefinition,
                                                       const Keire::ShaderGraphCompilation& previous,
        ...
        [[nodiscard]] bool ApplyParameterDefault(const Keire::ShaderGraphNode& node,
        ...
        [[nodiscard]] bool BuildParameterOnlyCompilation(const Keire::ShaderGraphDefinition& previousDefinition,
                                                         const Keire::ShaderGraphCompilation& previousCompilation,
                                                         const Keire::ShaderGraphDefinition& definition,
                                                         Keire::ShaderGraphCompilation& compilation)
        {
            ...
            for (std::size_t index = 0; index < definition.Nodes.size(); ++index)
            {
                ...
                if (before.Id == after.Id && before.Kind == Keire::ShaderGraphNodeKind::Parameter &&
                    after.Kind == Keire::ShaderGraphNodeKind::Parameter && before.Value != after.Value)
                {
                                  ...
                                  const Keire::ShaderGraphCompileOptions& options)
        {
            ...
            if (!importer.ContextualImport)
                throw std::logic_error("Live Shader Graph compilation requires the contextual shader importer.");
            ...
            for (const auto& variant : compilation.Variants)
            {
                                           ...
                                           readInclude = options.ReadInclude](const std::filesystem::path& requested)
                {
                    ...
                    throw std::runtime_error("Live Shader Graph shader dependency is unavailable: " +
        ...
        [[nodiscard]] StableNodeId CanvasPinType(const Keire::ShaderGraphValueType type) noexcept
        {
            ...
            if (type == Keire::ShaderGraphValueType::MaterialAttributes)
                return 3U;
            if (type == Keire::ShaderGraphValueType::Bsdf)
                return 4U;
            return 1U;
        }

        [[nodiscard]] std::string_view NodeCategory(const Keire::ShaderGraphNodeKind kind) noexcept
        {
            const auto* descriptor = Keire::FindShaderGraphNodeDescriptor(Keire::ShaderGraphNodeTypeId(kind));
            return descriptor ? descriptor->Category : std::string_view("Unknown");
        }

        [[nodiscard]] Keire::UiColor NodeColor(const Keire::ShaderGraphNodeKind kind) noexcept
        {
            switch (kind)
            {
        ...
        [[nodiscard]] const Keire::ShaderGraphNode& RequireNode(const Keire::ShaderGraphDefinition& definition,
        ...
        [[nodiscard]] const Keire::ShaderGraphPin& RequirePin(const Keire::ShaderGraphNode& node,
                                                              const Keire::AssetId id)
        {
            const auto found = std::ranges::find(node.Pins, id, &Keire::ShaderGraphPin::Id);
            if (found == node.Pins.end())
                throw std::invalid_argument("Shader Graph pin is unavailable.");
            return *found;
        }
    ...
    std::optional<Keire::AssetId> ShaderGraphCanvasModel::Node(const StableNodeId id) const noexcept
    {
        return ResolveCanvasIdentity(NodeIdentities, id);
    }

    std::optional<Keire::AssetId> ShaderGraphCanvasModel::Pin(const StableNodeId id) const noexcept
    {
        return ResolveCanvasIdentity(PinIdentities, id);
    }

    std::optional<Keire::AssetId> ShaderGraphCanvasModel::Connection(const StableNodeId id) const noexcept
    {
        return ResolveCanvasIdentity(ConnectionIdentities, id);
    }

    ShaderGraphDocument::ShaderGraphDocument(ShaderGraphDocumentSpecification specification)
                  ...
                  .Persist = m_Specification.Persist})
    {
        if (!m_Specification.Persist)
            throw std::invalid_argument("Shader Graph documents require a persistence callback.");
    }

    ShaderGraphDocument::~ShaderGraphDocument() noexcept
    {
    ...
    void ShaderGraphDocument::SetJobSystem(Keire::Ref<Keire::JobSystem> jobs)
    {
        if (m_BackgroundCompilation || m_JobScope)
            throw std::logic_error("Shader Graph jobs are already configured.");
        if (!jobs)
            throw std::invalid_argument("Shader Graph job system is unavailable.");
    ...
    void ShaderGraphDocument::Save()
    {
        RecompileCurrent(true);
        if (!Publishable())
            throw std::logic_error("Shader Graph cannot be saved until its generated-shader diagnostics are clear.");
        m_Host.Save();
    }

    void ShaderGraphDocument::Discard() { m_Host.Discard(); }

    void ShaderGraphDocument::Close() noexcept
    {
    ...
    bool ShaderGraphDocument::Redo() { return m_Host.Redo(); }

    std::string_view ShaderGraphDocument::Diagnostic() const noexcept
    {
        return m_Host.Diagnostic().empty() ? std::string_view(m_Diagnostic) : m_Host.Diagnostic();
    }

    bool ShaderGraphDocument::Edit(const std::string_view name,
                                   const std::function<void(Keire::ShaderGraphDefinition&)>& operation)
    {
        if (!operation)
            throw std::invalid_argument("Shader Graph edits require an operation.");
        auto candidate = m_Host.Draft();
        operation(candidate);
        return m_Host.Edit(name, std::move(candidate));
    }

    bool ShaderGraphDocument::AddNode(Keire::ShaderGraphNode node)
    {
        return Edit("Add Shader Graph node", [node = std::move(node)](auto& definition) mutable
                    { definition.Nodes.push_back(std::move(node)); });
    }

    bool ShaderGraphDocument::EditNode(const Keire::AssetId node,
                                       const std::function<void(Keire::ShaderGraphNode&)>& operation)
    {
        if (!operation)
            throw std::invalid_argument("Shader Graph node edits require an operation.");
        return Edit("Edit Shader Graph node",
    ...
    bool ShaderGraphDocument::AddConnection(Keire::ShaderGraphConnection connection)
    {
        ...
        return Edit("Connect Shader Graph pins",
    ...
    bool ShaderGraphDocument::RemoveConnection(const Keire::AssetId connection)
    {
        return Edit("Disconnect Shader Graph pins",
                    [connection](auto& definition)
                    {
                        const auto found =
                            std::ranges::find(definition.Connections, connection, &Keire::ShaderGraphConnection::Id);
                                                                       ...
                                                                       const Keire::ShaderGraphEndpoint input) const
    {
        ...
        try
        {
            ...
            if (output.Node == input.Node || outputPin.Direction != Keire::ShaderGraphPinDirection::Output ||
    ...
    ShaderGraphCanvasModel ShaderGraphDocument::BuildCanvasModel() const
    {
        ShaderGraphCanvasModel result;
        ...
        for (const auto& source : m_Host.Draft().Nodes)
        {
            ...
            for (const auto& sourcePin : source.Pins)
            {
                ...
                node.Pins.push_back({pinId, sourcePin.Name,
                                     sourcePin.Direction == Keire::ShaderGraphPinDirection::Input
    ...
    void ShaderGraphDocument::SetCompileOptions(Keire::ShaderGraphCompileOptions options)
    {
    ...
    void ShaderGraphDocument::SetPreviewSettings(ShaderGraphPreviewSettings settings)
    {
        if (settings.Mesh == Keire::ShaderGraphPreviewMesh::Custom && !settings.CustomMesh)
            throw std::invalid_argument("Custom Shader Graph preview requires a mesh asset.");
    ...
    bool ShaderGraphDocument::CompilationPending() const noexcept
    {
        return m_PendingDefinition.has_value() || static_cast<bool>(m_BackgroundCompilation);
    }

    void ShaderGraphDocument::ApplyLiveRevision() const
    {
    ...
    void ShaderGraphDocument::AdvanceCompilation(const double deltaSeconds)
    {
        if (!std::isfinite(deltaSeconds) || deltaSeconds < 0.0 || deltaSeconds > 60.0)
            throw std::invalid_argument("Shader Graph compilation delta must be finite and in the range 0..60.");
                     ...
                     !HasEquivalentRuntimeShaders(*previousDefinition, *previousCompilation, definition, compilation)))
                {
                    ...
                    catch (const std::exception& error)
                    {
                        compilation.Diagnostics.push_back(
                            {Keire::ShaderGraphDiagnosticSeverity::Error, "SG2001",
    ...
    void ShaderGraphDocument::ConsumeBackgroundCompilation(const bool wait)
    {
    ...
    void ShaderGraphDocument::EnsureJobScope()
    {
        if (m_JobScope)
            return;
        if (!m_JobSystem)
        ...
        m_JobScope = m_JobSystem->CreateScope("Shader Graph compilation");
    }

    void ShaderGraphDocument::CancelBackgroundCompilation() noexcept
    {
        if (m_BackgroundCompilation)
        {
    ...
    void ShaderGraphDocument::ApplyCompilation(Keire::ShaderGraphDefinition definition,
                                               Keire::ShaderGraphCompilation compilation,
                                               std::vector<Keire::Ref<Keire::ShaderAsset>> developmentShaders)
    {
        ...
        if (!m_Compilation.Succeeded())
        {
            ...
            {
                const auto error =
                    std::ranges::find(m_Compilation.Diagnostics, Keire::ShaderGraphDiagnosticSeverity::Error,
                                      &Keire::ShaderGraphDiagnostic::Severity);
                m_Diagnostic =
                                                ...
                                                const bool compileDevelopmentShaders)
    {
             ...
             !HasEquivalentRuntimeShaders(*m_LastGoodDefinition, *m_LastGoodCompilation, definition, compilation)))
        {
            ...
            {
                compilation.Diagnostics.push_back({Keire::ShaderGraphDiagnosticSeverity::Error, "SG2001",
                                                   std::string("Live shader compilation failed: ") + error.what()});
            }
        }
    ...
    void ShaderGraphDocument::RecompileCurrent(const bool compileDevelopmentShaders)
    {
        if (m_Host.IsOpen())
        {
            if (++m_RequestedGeneration == 0)

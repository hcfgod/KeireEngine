...
namespace KeireEditor
{
    namespace
    {
                                                                ...
                                                                const std::filesystem::path& confinedRoot)
        {
            if (relative.empty() || relative.is_absolute())
                throw std::invalid_argument("Material Graph publication paths must be project-relative.");
            ...
            if (error || !SameOrChild(confinedRoot, parent))
                throw std::invalid_argument("Material Graph publication path escaped its asset-owned directory.");
        ...
        void CopyMetadataIfPresent(const std::filesystem::path& published, const std::filesystem::path& staged)
        {
            ...
            if (!std::filesystem::exists(publishedMetadata, error))
            {
                if (error && error != std::errc::no_such_file_or_directory)
                    throw std::runtime_error("Cannot inspect generated Material Graph metadata: " + error.message());
                return;
            }
            if (!std::filesystem::is_regular_file(publishedMetadata, error) || error)
                throw std::runtime_error("Generated Material Graph metadata is not a regular file.");
            ...
            if (error)
                throw std::runtime_error("Cannot stage generated Material Graph metadata: " + error.message());
        }
                                                    ...
                                                    const std::error_code& rollbackError)
        {
            ...
            catch (...)
            {
                std::throw_with_nested(
                    std::runtime_error("Material Graph publication rollback failed: " + rollbackError.message()));
            }
        }
    } // namespace
    ...
    void PublishMaterialGraph(const MaterialGraphPublication& publication)
    {
        ...
        {
            throw std::invalid_argument("Material Graph publication requires a complete asset specification.");
        }

        std::error_code error;
        const auto projectRoot = std::filesystem::weakly_canonical(publication.ProjectRoot, error);
        if (error)
            throw std::runtime_error("Cannot resolve the Material Graph project root: " + error.message());
        ...
        if (error || !SameOrChild(projectRoot, sourceRoot))
            throw std::runtime_error("Cannot resolve the Material Graph source root.");

        const auto generatedRelative =
            publication.SourceDirectory / "Generated" / "MaterialGraphs" / publication.Asset.ToString();
        ...
        if (error || !SameOrChild(sourceRoot, generatedRoot))
            throw std::runtime_error("Cannot resolve the Material Graph generated-shader directory.");
        const auto graphSource = ResolveConfinedPath(sourceRoot, publication.GraphRelativePath, sourceRoot);
        if (graphSource.extension() != ".keirematerialgraph")
            throw std::invalid_argument("Material Graph publication requires a graph source path.");
        ...
        if (error)
            throw std::runtime_error("Cannot create the Material Graph transaction directory: " + error.message());
        ...
        for (const auto& variant : publication.Variants)
        {
            ...
            if (publishedSource.extension() != ".hlsl" ||
                !publishedSource.filename().string().starts_with("MaterialGraph-"))
            {
                throw std::invalid_argument("Generated Material Graph shader paths must use the owned naming scheme.");
            }
            ...
            if (!stagedFiles.insert(stagedSource).second || !stagedFiles.insert(stagedManifest).second)
                throw std::invalid_argument("Material Graph publication contains duplicate generated variants.");
        ...
        if (error)
            throw std::runtime_error("Cannot create the Material Graph generated-shader parent: " + error.message());

        const bool hadPrevious = std::filesystem::exists(generatedRoot, error);
        if (error)
            throw std::runtime_error("Cannot inspect the existing Material Graph publication: " + error.message());

...
void EditorWorkspaceLayer::DrawPerformanceOverlay(
    ...
    const std::optional<Keire::GpuOcclusionSurfaceDiagnostics> occlusionSurface)
{
    ...
    if (m_ShowAdvancedPerformanceOverlay && viewport.Size().Width >= 380.0F)
    {
        ...
        if (const auto renderer = Owner().Renderer())
        {
            ...
            occlusion =
                KeireEditor::BuildGpuOcclusionPanelDiagnostics(renderer->Capabilities(), statistics, occlusionSurface);
        }
        ...
        const float diagnosticsHeight =
            163.0F + diagnosticsLineHeight * static_cast<float>(statusLines.size() + visibilityLines.size()) + 8.0F;
        const float height = std::max(194.0F, diagnosticsHeight);
            ...
            if (occlusion)
            {
                const auto occlusionColor = occlusion->Warning ? m_Theme.Warning
                                            : occlusion->State == KeireEditor::GpuOcclusionDiagnosticState::Active
                                                ? m_Theme.Success
                                                : m_Theme.MutedText;
                float diagnosticsY = overlay.Minimum.Y + 163.0F;
                ...
                {
                    ui.DrawOverlayText({overlay.Minimum.X + horizontalPadding, diagnosticsY}, m_Theme.MutedText, line,
                                       diagnosticsFontSize, overlay);
                    diagnosticsY += diagnosticsLineHeight;
                }
            }

...
namespace KeireEditor
{
    namespace
    {
        ...
        [[nodiscard]] std::size_t ComponentCount(const Keire::MaterialGraphValueType type) noexcept
        {
            switch (type)
            {
        ...
        GraphValue(const Keire::MaterialGraphValue& value, const Keire::MaterialGraphValueType type,
                   const Keire::ShaderTextureSemantic semantic = Keire::ShaderTextureSemantic::Generic) noexcept
        {
            ...
            if (type == Keire::MaterialGraphValueType::MaterialAttributes ||
                type == Keire::MaterialGraphValueType::Bsdf)
        ...
        {
          public:
            GraphPreviewEvaluator(const Keire::MaterialGraphDefinition& definition,
            ...
            [[nodiscard]] std::optional<Keire::Vector4> MasterInput(const std::string_view name,
                                                                    const Keire::MaterialGraphValueType type)
            {
                if (!m_Master)
                    return std::nullopt;
                const auto pin = std::ranges::find(m_Master->Pins, name, &Keire::MaterialGraphPin::Name);
            ...
            [[nodiscard]] std::optional<PreviewMaterialSurface> MasterAttributes()
            {
                if (!m_Master)
                    return std::nullopt;
                const auto pin =
                    std::ranges::find(m_Master->Pins, "MaterialAttributes", &Keire::MaterialGraphPin::Name);
                if (pin == m_Master->Pins.end() || !m_Incoming.contains({m_Master->Id, pin->Id}))
                    return std::nullopt;
                try
                {
                    const auto value =
                        Coerce(Input(*m_Master, *pin), Keire::MaterialGraphValueType::MaterialAttributes);
          ...
          private:
            [[nodiscard]] PreviewGraphValue Coerce(PreviewGraphValue value,
                                                   const Keire::MaterialGraphValueType type) const
            {
                if (value.Type == type || ((value.Type == Keire::MaterialGraphValueType::Color &&
                                            type == Keire::MaterialGraphValueType::Vector4) ||
                                           (value.Type == Keire::MaterialGraphValueType::Vector4 &&
                                            type == Keire::MaterialGraphValueType::Color)))
                {
                    value.Type = type;
                    return value;
                }
                if ((value.Type == Keire::MaterialGraphValueType::Color ||
                     value.Type == Keire::MaterialGraphValueType::Vector4) &&
                    type == Keire::MaterialGraphValueType::Vector3)
                {
                    value.Type = type;
                    value.Data.W = 0.0F;
                    return value;
                }
                if (value.Type == Keire::MaterialGraphValueType::Vector3 &&
                    (type == Keire::MaterialGraphValueType::Color || type == Keire::MaterialGraphValueType::Vector4))
                {
                    value.Type = type;
                    value.Data.W = 1.0F;
                    return value;
                }
                if (value.Type != Keire::MaterialGraphValueType::Scalar ||
                    type == Keire::MaterialGraphValueType::Texture2D ||
                    type == Keire::MaterialGraphValueType::MaterialAttributes ||
                    type == Keire::MaterialGraphValueType::Bsdf)
            ...
            [[nodiscard]] PreviewGraphValue Input(const Keire::MaterialGraphNode& node,
            ...
            [[nodiscard]] PreviewGraphValue NamedInput(const Keire::MaterialGraphNode& node,
                                                       const std::string_view name)
            {
                const auto pin = std::ranges::find_if(
                    node.Pins,
                    [&](const Keire::MaterialGraphPin& candidate)
                    {
                        return candidate.Direction == Keire::MaterialGraphPinDirection::Input && candidate.Name == name;
                    });
            ...
            [[nodiscard]] PreviewGraphValue Binary(const Keire::MaterialGraphNode& node, Operation operation)
            {
            ...
            template <typename Operation>
            [[nodiscard]] PreviewGraphValue Unary(const Keire::MaterialGraphNode& node, Operation operation)
            {
                                                          ...
                                                          const Keire::Vector2 uv) const noexcept
            {
                ...
                return {.Data = sample, .Type = Keire::MaterialGraphValueType::Color};
            }

            [[nodiscard]] PreviewGraphValue Evaluate(const Keire::MaterialGraphEndpoint endpoint)
            {
                ...
                const auto outputPin = std::ranges::find(node.Pins, endpoint.Pin, &Keire::MaterialGraphPin::Id);
                if (outputPin == node.Pins.end() || outputPin->Direction != Keire::MaterialGraphPinDirection::Output)
                ...
                case Keire::MaterialGraphNodeKind::Parameter:
                {
                    ...
                    if (node.ValueType == Keire::MaterialGraphValueType::Texture2D)
                    {
                ...
                case Keire::MaterialGraphNodeKind::TextureSample:
                {
                    ...
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    result = TextureSample(texture, {uv.X, uv.Y});
                    if (outputPin->Name == "RGB")
                    {
                        result.Type = Keire::MaterialGraphValueType::Vector3;
                        result.Data.W = 0.0F;
                    }
                    else if (outputPin->Type == Keire::MaterialGraphValueType::Scalar)
                    {
                        const float component = outputPin->Name == "R"   ? result.Data.X
                                                : outputPin->Name == "G" ? result.Data.Y
                                                : outputPin->Name == "B" ? result.Data.Z
                                                                         : result.Data.W;
                        result = {.Data = {component, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    }
                    break;
                }
                case Keire::MaterialGraphNodeKind::UV:
                    result = {.Data = {m_Uv.X, m_Uv.Y, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Vector2};
                    break;
                case Keire::MaterialGraphNodeKind::UVTransform:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto tiling = Coerce(NamedInput(node, "Tiling"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto offset = Coerce(NamedInput(node, "Offset"), Keire::MaterialGraphValueType::Vector2).Data;
                    result = {.Data = {uv.X * tiling.X + offset.X, uv.Y * tiling.Y + offset.Y, 0.0F, 0.0F},
                              .Type = Keire::MaterialGraphValueType::Vector2};
                    break;
                }
                case Keire::MaterialGraphNodeKind::NormalMap:
                {
                    const auto sample = Coerce(NamedInput(node, "Sample"), Keire::MaterialGraphValueType::Color).Data;
                    const float scale = Coerce(NamedInput(node, "Scale"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    ...
                    const auto world = Normalize(
                        ...
                        m_Normal);
                    result = {.Data = {world.X, world.Y, world.Z, 0.0F},
                              .Type = Keire::MaterialGraphValueType::Vector3};
                    break;
                }
                case Keire::MaterialGraphNodeKind::DetailNormal:
                {
                    const auto base = Coerce(NamedInput(node, "Base"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto detail = Coerce(NamedInput(node, "Detail"), Keire::MaterialGraphValueType::Vector3).Data;
                    const float strength = std::clamp(
                        Coerce(NamedInput(node, "Strength"), Keire::MaterialGraphValueType::Scalar).Data.X, 0.0F, 1.0F);
                ...
                case Keire::MaterialGraphNodeKind::Parallax:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float height =
                        Coerce(NamedInput(node, "Height"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float scale = Coerce(NamedInput(node, "Scale"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const auto tangent = Normalize(Cross({0.0F, 1.0F, 0.0F}, m_Normal), {1.0F, 0.0F, 0.0F});
                    const auto bitangent = Normalize(Cross(m_Normal, tangent), {0.0F, 1.0F, 0.0F});
                ...
                case Keire::MaterialGraphNodeKind::Add:
                    result = Binary(node, std::plus<float>{});
                    break;
                case Keire::MaterialGraphNodeKind::Subtract:
                    result = Binary(node, std::minus<float>{});
                    break;
                case Keire::MaterialGraphNodeKind::Multiply:
                    result = Binary(node, std::multiplies<float>{});
                    break;
                case Keire::MaterialGraphNodeKind::Divide:
                    result = Binary(node, [](const float left, const float right) { return SafeDivide(left, right); });
                    break;
                case Keire::MaterialGraphNodeKind::Power:
                {
                ...
                case Keire::MaterialGraphNodeKind::Minimum:
                    result = Binary(node, [](const float left, const float right) { return std::min(left, right); });
                    break;
                case Keire::MaterialGraphNodeKind::Maximum:
                    result = Binary(node, [](const float left, const float right) { return std::max(left, right); });
                    break;
                case Keire::MaterialGraphNodeKind::Lerp:
                {
                    const auto left = Coerce(NamedInput(node, "A"), node.ValueType);
                    const auto right = Coerce(NamedInput(node, "B"), node.ValueType);
                    const float factor = Coerce(NamedInput(node, "T"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result.Type = node.ValueType;
                    for (std::size_t component = 0; component < ComponentCount(node.ValueType); ++component)
                        SetComponent(
                ...
                case Keire::MaterialGraphNodeKind::Length:
                {
                    ...
                    result = {.Data = {std::sqrt(lengthSquared), 0.0F, 0.0F, 0.0F},
                              .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::Dot:
                {
                    ...
                    result = {.Data = {dot, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::Remap:
                {
                    const auto value = Coerce(NamedInput(node, "Value"), node.ValueType);
                ...
                case Keire::MaterialGraphNodeKind::SmoothStep:
                {
                ...
                case Keire::MaterialGraphNodeKind::Step:
                {
                ...
                {
                    const auto normal = Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                    const float power = Coerce(NamedInput(node, "Power"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float reflectance =
                        Coerce(NamedInput(node, "F0"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float facing =
                    ...
                    const float value =
                        std::clamp(reflectance + (1.0F - reflectance) *
                                   ...
                                   0.0F, 1.0F);
                    result = {.Data = {value, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::VertexColor:
                    result = {.Data = {1.0F, 1.0F, 1.0F, 1.0F}, .Type = Keire::MaterialGraphValueType::Color};
                    break;
                ...
                case Keire::MaterialGraphNodeKind::ViewDirection:
                    result = {.Data = {0.0F, 0.0F, 1.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Vector3};
                    break;
                case Keire::MaterialGraphNodeKind::RotateUV:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto center = Coerce(NamedInput(node, "Center"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float rotation =
                        Coerce(NamedInput(node, "Rotation"), Keire::MaterialGraphValueType::Scalar).Data.X;
                ...
                case Keire::MaterialGraphNodeKind::SimpleNoise:
                {
                    ...
                    const float scale = Coerce(NamedInput(node, "Scale"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float detail =
                        Coerce(NamedInput(node, "Detail"), Keire::MaterialGraphValueType::Scalar).Data.X;
                ...
                {
                    const auto color = Coerce(NamedInput(node, "Color"), Keire::MaterialGraphValueType::Color).Data;
                    const float amount = std::clamp(
                        Coerce(NamedInput(node, "Amount"), Keire::MaterialGraphValueType::Scalar).Data.X, 0.0F, 1.0F);
                ...
                case Keire::MaterialGraphNodeKind::Posterize:
                {
                    const auto value = Coerce(NamedInput(node, "Value"), node.ValueType);
                    const float steps = std::max(
                        std::abs(Coerce(NamedInput(node, "Steps"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0F);
                    result.Type = node.ValueType;
                    for (std::size_t component = 0; component < ComponentCount(node.ValueType); ++component)
                        SetComponent(result.Data, component,
                ...
                case Keire::MaterialGraphNodeKind::Round:
                    result = Unary(node, [](const float value) { return std::round(value); });
                    break;
                case Keire::MaterialGraphNodeKind::Truncate:
                    result = Unary(node, [](const float value) { return std::trunc(value); });
                    break;
                case Keire::MaterialGraphNodeKind::Sign:
                    result = Unary(node,
                                   [](const float value) { return value < 0.0F   ? -1.0F
                                                                  : value > 0.0F ? 1.0F
                                                                                 : 0.0F; });
                    break;
                case Keire::MaterialGraphNodeKind::Modulo:
                    result = Binary(node, [](const float left, const float right)
                                    { return std::fmod(left, std::abs(right) > 1.0e-6F ? right : 1.0e-6F); });
                    break;
                case Keire::MaterialGraphNodeKind::SquareRoot:
                    result = Unary(node, [](const float value) { return std::sqrt(std::max(value, 0.0F)); });
                    break;
                case Keire::MaterialGraphNodeKind::ReciprocalSquareRoot:
                    result = Unary(node, [](const float value) { return 1.0F / std::sqrt(std::max(value, 1.0e-8F)); });
                    break;
                case Keire::MaterialGraphNodeKind::Exponential2:
                    result = Unary(node, [](const float value) { return std::exp2(value); });
                    break;
                case Keire::MaterialGraphNodeKind::Logarithm2:
                    result =
                        Unary(node, [](const float value) { return std::log2(std::max(std::abs(value), 1.0e-8F)); });
                    break;
                case Keire::MaterialGraphNodeKind::Tangent:
                    result = Unary(node, [](const float value) { return std::tan(value); });
                    break;
                case Keire::MaterialGraphNodeKind::ArcSine:
                    result = Unary(node, [](const float value) { return std::asin(std::clamp(value, -1.0F, 1.0F)); });
                    break;
                case Keire::MaterialGraphNodeKind::ArcCosine:
                    result = Unary(node, [](const float value) { return std::acos(std::clamp(value, -1.0F, 1.0F)); });
                    break;
                case Keire::MaterialGraphNodeKind::ArcTangent2:
                    result = Binary(node, [](const float left, const float right) { return std::atan2(left, right); });
                    break;
                case Keire::MaterialGraphNodeKind::DerivativeX:
                case Keire::MaterialGraphNodeKind::DerivativeY:
                case Keire::MaterialGraphNodeKind::FilterWidth:
                    result = {.Type = node.ValueType};
                    break;
                case Keire::MaterialGraphNodeKind::Cross:
                {
                    const auto left = Coerce(NamedInput(node, "A"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto right = Coerce(NamedInput(node, "B"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto value = Cross({left.X, left.Y, left.Z}, {right.X, right.Y, right.Z});
                    result = {.Data = {value.X, value.Y, value.Z, 0.0F},
                              .Type = Keire::MaterialGraphValueType::Vector3};
                    break;
                }
                case Keire::MaterialGraphNodeKind::Distance:
                {
                    const auto left = Coerce(NamedInput(node, "A"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto right = Coerce(NamedInput(node, "B"), Keire::MaterialGraphValueType::Vector3).Data;
                ...
                case Keire::MaterialGraphNodeKind::Reflect:
                {
                    const auto incident = Coerce(NamedInput(node, "A"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto normalValue = Coerce(NamedInput(node, "B"), Keire::MaterialGraphValueType::Vector3).Data;
                ...
                case Keire::MaterialGraphNodeKind::Refract:
                {
                    const auto incidentValue =
                        Coerce(NamedInput(node, "Incident"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto normalValue =
                        Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto incident = Normalize({incidentValue.X, incidentValue.Y, incidentValue.Z});
                    const auto normal = Normalize({normalValue.X, normalValue.Y, normalValue.Z});
                    const float ior = std::max(
                        std::abs(Coerce(NamedInput(node, "IOR"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0e-4F);
                ...
                case Keire::MaterialGraphNodeKind::AppendVector:
                {
                    const auto xyz = Coerce(NamedInput(node, "XYZ"), Keire::MaterialGraphValueType::Vector3).Data;
                    const float w = Coerce(NamedInput(node, "W"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result = {.Data = {xyz.X, xyz.Y, xyz.Z, w}, .Type = Keire::MaterialGraphValueType::Vector4};
                    break;
                }
                case Keire::MaterialGraphNodeKind::ComponentMask:
                {
                    const auto value = Coerce(NamedInput(node, "Value"), Keire::MaterialGraphValueType::Vector4).Data;
                ...
                case Keire::MaterialGraphNodeKind::UV1:
                case Keire::MaterialGraphNodeKind::ScreenPosition:
                    result = {.Data = {m_Uv.X, m_Uv.Y, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Vector2};
                    break;
                case Keire::MaterialGraphNodeKind::WorldTangent:
                {
                    const auto tangent = Normalize(Cross({0.0F, 1.0F, 0.0F}, m_Normal), {1.0F, 0.0F, 0.0F});
                ...
                case Keire::MaterialGraphNodeKind::CameraPosition:
                    result = {.Data = {0.0F, 0.0F, 3.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Vector3};
                    break;
                case Keire::MaterialGraphNodeKind::ObjectPosition:
                    result = {.Data = {0.0F, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Vector3};
                    break;
                case Keire::MaterialGraphNodeKind::Time:
                case Keire::MaterialGraphNodeKind::DeltaTime:
                    result = {.Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                case Keire::MaterialGraphNodeKind::DepthFade:
                {
                    const float distance =
                        Coerce(NamedInput(node, "Distance"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float fadeDistance = std::max(
                        std::abs(
                            Coerce(NamedInput(node, "Fade Distance"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0e-4F);
                    result = {.Data = {std::clamp(distance / fadeDistance, 0.0F, 1.0F), 0.0F, 0.0F, 0.0F},
                              .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::Luminance:
                {
                    const auto color = Coerce(NamedInput(node, "Color"), Keire::MaterialGraphValueType::Color).Data;
                ...
                case Keire::MaterialGraphNodeKind::HueShift:
                {
                    const auto color = Coerce(NamedInput(node, "Color"), Keire::MaterialGraphValueType::Color).Data;
                    const float angle =
                        Coerce(NamedInput(node, "Shift"), Keire::MaterialGraphValueType::Scalar).Data.X *
                        6.28318530718F;
                ...
                case Keire::MaterialGraphNodeKind::Checkerboard:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto colorA = Coerce(NamedInput(node, "Color A"), Keire::MaterialGraphValueType::Color).Data;
                    const auto colorB = Coerce(NamedInput(node, "Color B"), Keire::MaterialGraphValueType::Color).Data;
                    const auto scale = Coerce(NamedInput(node, "Scale"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto parity =
                        static_cast<std::int64_t>(std::floor(uv.X * scale.X) + std::floor(uv.Y * scale.Y)) & 1;
                    result = {.Data = parity != 0 ? colorB : colorA, .Type = Keire::MaterialGraphValueType::Color};
                    break;
                }
                ...
                case Keire::MaterialGraphNodeKind::Panner:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto speed = Coerce(NamedInput(node, "Speed"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float time = Coerce(NamedInput(node, "Time"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result = {.Data = {uv.X + speed.X * time, uv.Y + speed.Y * time, 0.0F, 0.0F},
                              .Type = Keire::MaterialGraphValueType::Vector2};
                    break;
                }
                case Keire::MaterialGraphNodeKind::PolarCoordinates:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto center = Coerce(NamedInput(node, "Center"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float radialScale =
                        Coerce(NamedInput(node, "Radial Scale"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float lengthScale =
                        Coerce(NamedInput(node, "Length Scale"), Keire::MaterialGraphValueType::Scalar).Data.X;
                ...
                case Keire::MaterialGraphNodeKind::SphereMask:
                {
                    const auto first = Coerce(NamedInput(node, "A"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto second = Coerce(NamedInput(node, "B"), Keire::MaterialGraphValueType::Vector3).Data;
                    const float x = first.X - second.X;
                    const float y = first.Y - second.Y;
                    const float z = first.Z - second.Z;
                    const float radius = std::max(
                        std::abs(Coerce(NamedInput(node, "Radius"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0e-5F);
                    const float hardness = std::max(
                        std::abs(Coerce(NamedInput(node, "Hardness"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0F);
                    const float mask =
                        std::clamp((1.0F - std::sqrt(x * x + y * y + z * z) / radius) * hardness, 0.0F, 1.0F);
                    result = {.Data = {mask, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::RadialGradient:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto center = Coerce(NamedInput(node, "Center"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float radius =
                        Coerce(NamedInput(node, "Radius"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float density = std::max(
                        std::abs(Coerce(NamedInput(node, "Density"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0e-4F);
                    const float x = uv.X - center.X;
                    const float y = uv.Y - center.Y;
                    const float gradient = std::clamp((radius - std::sqrt(x * x + y * y)) * density, 0.0F, 1.0F);
                    result = {.Data = {gradient, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::LinearGradient:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto direction =
                        Coerce(NamedInput(node, "Direction"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float offset =
                        Coerce(NamedInput(node, "Offset"), Keire::MaterialGraphValueType::Scalar).Data.X;
                ...
                case Keire::MaterialGraphNodeKind::Contrast:
                {
                    const auto color = Coerce(NamedInput(node, "Color"), Keire::MaterialGraphValueType::Color).Data;
                    const float contrast =
                        Coerce(NamedInput(node, "Contrast"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float pivot = Coerce(NamedInput(node, "Pivot"), Keire::MaterialGraphValueType::Scalar).Data.X;
                ...
                case Keire::MaterialGraphNodeKind::Saturation:
                {
                    const auto color = Coerce(NamedInput(node, "Color"), Keire::MaterialGraphValueType::Color).Data;
                    const float saturation =
                        Coerce(NamedInput(node, "Saturation"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float luminance = color.X * 0.2126F + color.Y * 0.7152F + color.Z * 0.0722F;
                ...
                case Keire::MaterialGraphNodeKind::BlendOverlay:
                {
                    const auto base = Coerce(NamedInput(node, "Base"), Keire::MaterialGraphValueType::Color).Data;
                    const auto blend = Coerce(NamedInput(node, "Blend"), Keire::MaterialGraphValueType::Color).Data;
                    const float opacity = std::clamp(
                        Coerce(NamedInput(node, "Opacity"), Keire::MaterialGraphValueType::Scalar).Data.X, 0.0F, 1.0F);
                ...
                case Keire::MaterialGraphNodeKind::Blackbody:
                {
                    const float kelvin =
                        std::clamp(
                            Coerce(NamedInput(node, "Temperature"), Keire::MaterialGraphValueType::Scalar).Data.X,
                            1000.0F, 40000.0F) *
                        0.01F;
                    const float red =
                        kelvin <= 66.0F ? 1.0F
                    ...
                    const float blue =
                        ...
                        : kelvin <= 19.0F
                            ...
                            : std::clamp(0.543206811F * std::log(std::max(kelvin - 10.0F, 1.0F)) - 1.19625409F, 0.0F,
                                         1.0F);
                    result = {.Data = {red, green, blue, 1.0F}, .Type = Keire::MaterialGraphValueType::Color};
                    break;
                }
                case Keire::MaterialGraphNodeKind::ReflectionVector:
                {
                    const auto normalValue =
                        Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                ...
                case Keire::MaterialGraphNodeKind::FacingRatio:
                {
                    const auto normalValue =
                        Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto normal = Normalize({normalValue.X, normalValue.Y, normalValue.Z}, m_Normal);
                    const float power = std::max(
                        std::abs(Coerce(NamedInput(node, "Power"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0e-4F);
                    const float ratio = std::pow(std::clamp(1.0F - normal.Z, 0.0F, 1.0F), power);
                    result = {.Data = {ratio, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::Dither:
                {
                    const float alpha = std::clamp(
                        Coerce(NamedInput(node, "Alpha"), Keire::MaterialGraphValueType::Scalar).Data.X, 0.0F, 1.0F);
                    const auto screen =
                        Coerce(NamedInput(node, "Screen Position"), Keire::MaterialGraphValueType::Vector2).Data;
                ...
                case Keire::MaterialGraphNodeKind::Wave:
                {
                    const auto uv = Coerce(NamedInput(node, "UV"), Keire::MaterialGraphValueType::Vector2).Data;
                    const auto direction =
                        Coerce(NamedInput(node, "Direction"), Keire::MaterialGraphValueType::Vector2).Data;
                    const float frequency =
                        Coerce(NamedInput(node, "Frequency"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float phase = Coerce(NamedInput(node, "Phase"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float inverseLength =
                        1.0F / std::max(std::sqrt(direction.X * direction.X + direction.Y * direction.Y), 1.0e-5F);
                    const float wave = std::sin((uv.X * direction.X + uv.Y * direction.Y) * inverseLength * frequency *
                                           ...
                                           0.5F +
                                       0.5F;
                    result = {.Data = {wave, 0.0F, 0.0F, 0.0F}, .Type = Keire::MaterialGraphValueType::Scalar};
                    break;
                }
                case Keire::MaterialGraphNodeKind::TriplanarSample:
                {
                    const auto texture = NamedInput(node, "Texture");
                    const auto position =
                        Coerce(NamedInput(node, "Position"), Keire::MaterialGraphValueType::Vector3).Data;
                    const auto normalValue =
                        Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                    const float scale = Coerce(NamedInput(node, "Scale"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float sharpness = std::max(
                        std::abs(
                            Coerce(NamedInput(node, "Blend Sharpness"), Keire::MaterialGraphValueType::Scalar).Data.X),
                        1.0F);
                    ...
                    if (outputPin->Name == "RGB")
                        result = {.Data = sample, .Type = Keire::MaterialGraphValueType::Vector3};
                    else if (outputPin->Type == Keire::MaterialGraphValueType::Scalar)
                    {
                        ...
                        result = {.Data = sample, .Type = Keire::MaterialGraphValueType::Color};
                    break;
                }
                ...
                case Keire::MaterialGraphNodeKind::HeightToNormal:
                {
                    const float height =
                        Coerce(NamedInput(node, "Height"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const float strength =
                        Coerce(NamedInput(node, "Strength"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const auto normal =
                ...
                case Keire::MaterialGraphNodeKind::FlattenNormal:
                {
                    const auto normalValue =
                        Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                    const float strength = std::clamp(
                ...
                case Keire::MaterialGraphNodeKind::MakeMaterialAttributes:
                {
                    ...
                    surface.BaseColor =
                        Coerce(NamedInput(node, "BaseColor"), Keire::MaterialGraphValueType::Color).Data;
                    surface.Metallic =
                        Coerce(NamedInput(node, "Metallic"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.Roughness =
                        Coerce(NamedInput(node, "Roughness"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.Specular =
                        Coerce(NamedInput(node, "Specular"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.ClearCoat =
                        Coerce(NamedInput(node, "ClearCoat"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.ClearCoatRoughness =
                        Coerce(NamedInput(node, "ClearCoatRoughness"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.SheenColor =
                        Coerce(NamedInput(node, "SheenColor"), Keire::MaterialGraphValueType::Color).Data;
                    surface.SheenRoughness =
                        Coerce(NamedInput(node, "SheenRoughness"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const auto normal = Coerce(NamedInput(node, "Normal"), Keire::MaterialGraphValueType::Vector3).Data;
                    surface.Normal = {normal.X, normal.Y, normal.Z};
                    surface.Emission = Coerce(NamedInput(node, "Emission"), Keire::MaterialGraphValueType::Color).Data;
                    surface.Occlusion =
                        Coerce(NamedInput(node, "Occlusion"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.Opacity = Coerce(NamedInput(node, "Opacity"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.SubsurfaceColor =
                        Coerce(NamedInput(node, "SubsurfaceColor"), Keire::MaterialGraphValueType::Color).Data;
                    surface.Subsurface =
                        Coerce(NamedInput(node, "Subsurface"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.Anisotropy =
                        Coerce(NamedInput(node, "Anisotropy"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    const auto tangent =
                        Coerce(NamedInput(node, "Tangent"), Keire::MaterialGraphValueType::Vector3).Data;
                    surface.Tangent = {tangent.X, tangent.Y, tangent.Z};
                    surface.Transmission =
                        Coerce(NamedInput(node, "Transmission"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.IndexOfRefraction =
                        Coerce(NamedInput(node, "IndexOfRefraction"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.Refraction =
                        Coerce(NamedInput(node, "Refraction"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.Thickness =
                        Coerce(NamedInput(node, "Thickness"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result = {.Type = Keire::MaterialGraphValueType::MaterialAttributes, .Surface = surface};
                    break;
                }
                case Keire::MaterialGraphNodeKind::BreakMaterialAttributes:
                {
                    const auto attributes =
                        Coerce(NamedInput(node, "Attributes"), Keire::MaterialGraphValueType::MaterialAttributes);
                ...
                {
                    const auto first = Coerce(NamedInput(node, "A"), Keire::MaterialGraphValueType::MaterialAttributes)
                                           .Surface.value_or(PreviewMaterialSurface{});
                    const auto second = Coerce(NamedInput(node, "B"), Keire::MaterialGraphValueType::MaterialAttributes)
                                            .Surface.value_or(PreviewMaterialSurface{});
                    const float alpha = std::clamp(
                        Coerce(NamedInput(node, "Alpha"), Keire::MaterialGraphValueType::Scalar).Data.X, 0.0F, 1.0F);
                ...
                case Keire::MaterialGraphNodeKind::StandardSurfaceBsdf:
                {
                    ...
                    surface.Emission = Coerce(NamedInput(node, "Emission"), Keire::MaterialGraphValueType::Color).Data;
                    surface.Opacity = Coerce(NamedInput(node, "Opacity"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result = {.Type = Keire::MaterialGraphValueType::Bsdf, .Surface = surface};
                    break;
                }
                case Keire::MaterialGraphNodeKind::ClearCoatBsdf:
                {
                    auto surface = Coerce(NamedInput(node, "Base"), Keire::MaterialGraphValueType::Bsdf)
                                       .Surface.value_or(PreviewMaterialSurface{});
                    surface.ClearCoat =
                        Coerce(NamedInput(node, "Weight"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    surface.ClearCoatRoughness =
                        Coerce(NamedInput(node, "Roughness"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result = {.Type = Keire::MaterialGraphValueType::Bsdf, .Surface = surface};
                    break;
                ...
                case Keire::MaterialGraphNodeKind::SubsurfaceBsdf:
                {
                    auto surface = Coerce(NamedInput(node, "Base"), Keire::MaterialGraphValueType::Bsdf)
                                       .Surface.value_or(PreviewMaterialSurface{});
                    surface.SubsurfaceColor =
                        Coerce(NamedInput(node, "Color"), Keire::MaterialGraphValueType::Color).Data;
                    surface.Subsurface =
                        Coerce(NamedInput(node, "Weight"), Keire::MaterialGraphValueType::Scalar).Data.X;
                    result = {.Type = Keire::MaterialGraphValueType::Bsdf, .Surface = surface};
                    break;
                ...
                {
                    result = Coerce(NamedInput(node, "BSDF"), Keire::MaterialGraphValueType::Bsdf);
                    result.Type = Keire::MaterialGraphValueType::MaterialAttributes;
                    break;
                }
                ...
                case Keire::MaterialGraphNodeKind::StaticSwitch:
                {
                    const float condition =
                        Coerce(NamedInput(node, "Condition"), Keire::MaterialGraphValueType::Scalar).Data.X;
            ...
            const Keire::MaterialGraphDefinition& m_Definition;
                                                      ...
                                                      const Keire::Vector3 position = {})
        {
            ...
            if (evaluator)
            {
                ...
                else
                {
                    ...
                    if (const auto value = evaluator->MasterInput(colorName, Keire::MaterialGraphValueType::Color))
                        result.BaseColor = *value;
                    if (const auto value = evaluator->MasterInput("Emission", Keire::MaterialGraphValueType::Color))
                        result.Emission = {value->X, value->Y, value->Z};
                    if (const auto value = evaluator->MasterInput("Metallic", Keire::MaterialGraphValueType::Scalar))
                        result.Metallic = value->X;
                    if (const auto value = evaluator->MasterInput("Roughness", Keire::MaterialGraphValueType::Scalar))
                        result.Roughness = value->X;
                    if (const auto value = evaluator->MasterInput("Specular", Keire::MaterialGraphValueType::Scalar))
                        result.Specular = value->X;
                    if (const auto value = evaluator->MasterInput("ClearCoat", Keire::MaterialGraphValueType::Scalar))
                        result.ClearCoat = value->X;
                    if (const auto value =
                            evaluator->MasterInput("ClearCoatRoughness", Keire::MaterialGraphValueType::Scalar))
                        result.ClearCoatRoughness = value->X;
                    if (const auto value = evaluator->MasterInput("SheenColor", Keire::MaterialGraphValueType::Color))
                        result.SheenColor = {value->X, value->Y, value->Z};
                    if (const auto value =
                            evaluator->MasterInput("SheenRoughness", Keire::MaterialGraphValueType::Scalar))
                        result.SheenRoughness = value->X;
                    if (const auto value = evaluator->MasterInput("Opacity", Keire::MaterialGraphValueType::Scalar))
                        result.Opacity = value->X;
                    if (const auto value = evaluator->MasterInput("Occlusion", Keire::MaterialGraphValueType::Scalar))
                        result.Occlusion = value->X;
                    if (const auto value = evaluator->MasterInput("Normal", Keire::MaterialGraphValueType::Vector3))
                    {
        ...
        void Rasterize(const PreviewGeometry& geometry, const MaterialGraphPreviewRequest& request,

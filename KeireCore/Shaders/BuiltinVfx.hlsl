...
RWStructuredBuffer<uint> VisibilityOutputIndices : register(u8, space1);
RWByteAddressBuffer VisibilityOutputIndirectArguments : register(u9, space1);
RWStructuredBuffer<VfxInstanceData> VisibilityOutputInstances : register(u10, space1);
RWStructuredBuffer<VfxOcclusionCandidate> VisibilityCandidates : register(u11, space1);
RWStructuredBuffer<VfxInstanceData> VisibilityCandidateInstances : register(u12, space1);
RWStructuredBuffer<uint> VisibilityMask : register(u13, space1);
...
cbuffer VfxVisibilityDispatch : register(b1, space2)
{
...
static const uint VfxRendererSprite = 0;
static const uint VfxRendererMesh = 1;
static const uint VfxVisibilityConsumer = 1;
static const uint VfxForceVisible = 1;
                                 ...
                                 float maximumWorldPositionDisplacementRadius)
{
    ...
    VisibilityCandidates[candidateIndex] = candidate;
    VisibilityCandidateInstances[candidateIndex] = instance;
}

[numthreads(256, 1, 1)] void CSBuildVisibilityCandidates(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    const uint sourceCount = min(FilteredIndirectArguments.Load(4), VisibilityExecution.y);
    if (VisibilityExecution.z == VfxRendererRibbon)
    {
        ...
        [loop] for (uint source = 0U; source < sourceCount; ++source)
        {
            ...
            {
                const GpuParticle previous = Particles[particle.Identity.z - 1U];
    ...
    {
        const VfxInstanceData instance = BuildVfxInstance(particle);
...
[numthreads(1, 1, 1)] void CSCompactVisibility(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    const bool ribbonVisible =
        VisibilityExecution.z != VfxRendererRibbon || VisibilityMask[VisibilityMetadata.y] != 0U;
    uint outputCount = 0U;
    [loop] for (uint source = 0U; source < sourceCount; ++source)
    {
        const bool visible = ribbonVisible &&
                             (VisibilityExecution.z == VfxRendererRibbon ||
                              (source < VisibilityMetadata.z &&
                               VisibilityMask[VisibilityMetadata.y + source] != 0U));
        if (!visible)
            continue;
        const uint particleIndex = FilteredAliveIndices[source];
        VisibilityOutputIndices[outputCount] = particleIndex;
        if (VisibilityExecution.z != VfxRendererRibbon)
            VisibilityOutputInstances[outputCount] = BuildVfxInstance(Particles[particleIndex]);
        ++outputCount;
    }
    VisibilityOutputIndirectArguments.Store4(0, uint4(VisibilityMetadata.w, outputCount, 0U, 0U));
    VisibilityOutputIndirectArguments.Store(16, 0U);
}

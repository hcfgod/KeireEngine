...
#include "KeireInternal/Scenes/SceneStateStorage.h"
...
namespace Keire
{
    namespace Detail
    {
        ...
        bool SceneState::Destroy(const EntityId id, const float delaySeconds)
        {
            RequireOwner("DestroyEntity");
            if (!std::isfinite(delaySeconds) || delaySeconds < 0.0F)
                throw std::invalid_argument("Destruction delay must be finite and non-negative.");
            if (delaySeconds == 0.0F)
                return Destroy(id);
            if (!Contains(id))
                return false;
            const double deadline = m_Impl->DestructionTime + delaySeconds;
            const auto [entry, inserted] = m_Impl->DelayedDestruction.emplace(id, deadline);
            if (!inserted)
                entry->second = std::min(entry->second, deadline);
            return true;
        }
        ...
        bool SceneState::Destroy(const EntityId id)
        {
            ...
            const auto commit = [this, id]
            {
                ...
                for (const auto current : removed)
                {
                    ...
                    m_Impl->DelayedDestruction.erase(current);
        ...
        void SceneState::Update(const float deltaSeconds)
        {
            ...
            if (!std::isfinite(deltaSeconds) || deltaSeconds < 0.0F)
                throw std::invalid_argument("Scene update delta must be finite and non-negative.");
            m_Impl->DestructionTime += deltaSeconds;
            m_Impl->Traverse(
                [&]
                {
                    for (const auto& component : m_Impl->LifecycleComponents())
            ...
            // Collect first: destruction callbacks may schedule or remove other entities.
            std::vector<EntityId> expired;
            for (const auto& [entity, deadline] : m_Impl->DelayedDestruction)
                if (deadline <= m_Impl->DestructionTime)
                    expired.push_back(entity);
            for (const auto entity : expired)
                m_Impl->DelayedDestruction.erase(entity);
            for (const auto entity : expired)
                (void)Destroy(entity);
            FlushDeferred();
        }
        ...
        void SceneState::EndPlay() noexcept
        {
            ...
            m_Impl->DelayedDestruction.clear();
            m_Impl->DestructionTime = 0.0;
        }

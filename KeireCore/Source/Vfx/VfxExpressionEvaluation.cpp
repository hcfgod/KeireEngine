...
namespace Keire::Internal
{
    ...
    [[nodiscard]] std::optional<VfxParameterValue>
    EvaluateVfxPureExpression(const VfxValueOpcode opcode, const std::span<const VfxParameterValue* const> inputs,
                              const VfxValueType outputType, const bool clampRemap,
                              const VfxComparisonCondition comparison, const std::uint32_t outputIndex) noexcept
    {
        ...
        case VfxValueOpcode::Select:
            if (inputs.size() == 3 && inputs[0] && std::holds_alternative<bool>(*inputs[0]) && inputs[1] && inputs[2])
            {
        ...
        case VfxValueOpcode::PolarToRectangular:
        {
            ...
            return Vector2{FiniteOrZero(std::cos(radians) * *distance), FiniteOrZero(std::sin(radians) * *distance)};
        }
        ...
        case VfxValueOpcode::Rotate3D:
        {
            ...
            const Vector3 tangent{position->X - projection.X, position->Y - projection.Y, position->Z - projection.Z};
        ...
        case VfxValueOpcode::CellularNoise:
        {
            ...
            return SampleNoiseDerivative(kind, *coordinate, *frequency, *octaves, *roughness, *lacunarity, *range, 0U);
        }
        ...
        case VfxValueOpcode::CellularCurlNoise:
        {
            ...
            if (!coordinate || !frequency || !octaves || !roughness || !lacunarity || !amplitude || outputIndex != 0)
            {
                return std::nullopt;
            }
                                ...
                                const std::span<VfxParameterValue> registers) noexcept
    {
        ...
        try
        {
            for (const auto& instruction : program.ValueInstructions)
            {
                ...
                {
                    output = EvaluateVfxPureExpression(
                        instruction.Opcode,
                        std::span<const VfxParameterValue* const>(inputs.data(), instruction.Inputs.size()),

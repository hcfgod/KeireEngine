    ...
    namespace Detail
    {
                                                   ...
                                                   const std::map<AssetId, std::filesystem::path>& designTimeProjects)
        {
            ...
            std::string text =
                ...
                "# Visual Studio 2022 design-time solution. Runtime builds use .csproj files (net10.0/C# 14.0).\n";
            ...
            for (const auto& assembly : request.Assemblies)
            {
                ...
                const auto& projectName = names.at(assembly.Asset);
                const auto found = designTimeProjects.find(assembly.Asset);
                const auto projectPath = found == designTimeProjects.end()
                                             ? std::filesystem::path(projectName + ".csproj")
                                             : found->second.lexically_relative(projectRoot);
                text += "Project(\"" + std::string(csharpProject) + "\") = \"" + projectName + "\", \"" +
                        PathText(projectPath) + "\", \"" + guid + "\"\nEndProject\n";
            }
            text += "Global\n"
        ...
        [[nodiscard]] std::string GenerateManagedIdeAggregator(const ManagedBuildRequest& request,
                                                               const std::string_view targetFramework)
        {
            std::string text = "<Project Sdk=\"Microsoft.NET.Sdk\">\n  <PropertyGroup>\n"
                               "    <TargetFramework>" +
                               XmlEscape(targetFramework) +
                               ...
                               "  </PropertyGroup>\n  <ItemGroup>\n";
            for (const auto& assembly : request.Assemblies)
                text += "    <ProjectReference Include=\"" + XmlEscape(assembly.Definition.Name) +
                        ".VisualStudio.csproj\" />\n";
            text += "  </ItemGroup>\n</Project>\n";
            return text;
        }
        ...
        {
            auto designTimeNames = names;
            for (auto& [id, name] : designTimeNames)
                name += ".VisualStudio";
            auto text = GenerateProject(assembly, designTimeNames, projectRoot, projectDirectory, managedApi,
                                        managedApiDesignTimeProject, managedEditorApi, managedGenerator,
                                        assembly.Definition.Classification == ManagedAssemblyClassification::Editor,
                                        targetFramework, languageVersion);
            constexpr std::string_view sdk = "<Project Sdk=\"Microsoft.NET.Sdk\">";
            // Set paths before Sdk.props: NuGet reads MSBuildProjectExtensionsPath during that import.
            text.replace(text.find(sdk), sdk.size(),
                         "<Project>\n  <PropertyGroup>\n"
                         "    <BaseIntermediateOutputPath>obj/$(MSBuildProjectName)/</BaseIntermediateOutputPath>\n"
                         "    <BaseOutputPath>bin/$(MSBuildProjectName)/</BaseOutputPath>\n"
                         "  </PropertyGroup>\n  <Import Project=\"Sdk.props\" Sdk=\"Microsoft.NET.Sdk\" />");
            text.insert(text.rfind("</Project>"), "  <Import Project=\"Sdk.targets\" Sdk=\"Microsoft.NET.Sdk\" />\n");
            return text;
        }

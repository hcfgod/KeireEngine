...
export async function authenticate(request: Request): Promise<Caller> {
    ...
    return {
        ...
        admin: createClient(url, adminKey, {
            auth: { autoRefreshToken: false, persistSession: false },
            global: { headers: { "X-Client-Info": "keire-marketplace-edge/0.4.2" } },
        }),
    };
}
...
export function databaseFailure(error: { message: string; code?: string }): RequestError {
    const mappings = new Map<string, readonly [number, string, string]>([
        ...
        ["publisher_not_manageable", [403, "publisher.publisher_forbidden", "Your account cannot publish for this publisher."]],
        ["publisher_product_metadata_invalid", [400, "publisher.product_metadata_invalid", "Choose an active category and approved license."]],
        ["publisher_product_name_conflict", [409, "publisher.product_name_conflict", "That product name conflicts with an existing product. Choose a more specific name."]],
        ["publisher_published_product_name_immutable", [409, "publisher.product_name_immutable", "A published product must keep its existing name."]],
        ["publisher_published_product_metadata_immutable", [409, "publisher.product_metadata_immutable", "Published product metadata cannot change through a package upload."]],
        ...
        ["marketplace_signing_approval_withdrawal_requires_administrator", [403, "staff.signing_approval_withdrawal_forbidden", "Only an administrator can withdraw an approval that is waiting for offline signing."]],
        ["marketplace_publication_invalid", [400, "publication.manifest_invalid", "The signed publication fields are invalid."]],
        ["marketplace_signed_manifest_invalid", [400, "publication.manifest_invalid", "The signed publication envelope is invalid."]],
        ["marketplace_signature_key_untrusted", [409, "publication.key_untrusted", "The marketplace signing key is not trusted."]],
        ["marketplace_version_not_publishable", [409, "publication.not_ready", "This version is not approved for publication."]],
        ["marketplace_submission_not_publishable", [409, "publication.not_ready", "This submission is not approved for publication."]],
        ["marketplace_publication_validation_mismatch", [409, "publication.evidence_mismatch", "The offline signature does not match the passing validation report."]],
        ["marketplace_publication_source_missing", [409, "publication.source_unavailable", "The exact validated quarantine object is unavailable."]],
        ["marketplace_publication_artifact_mismatch", [409, "publication.evidence_mismatch", "The promoted release does not match the approved artifact."]],

...
`UiSystem` is an application-owned private implementation. `UiMode::Disabled` preserves the pre-UI runtime, `Headless` creates a deterministic context without platform or graphics state, and `Rendered` transactionally creates an SDL_GPU device, claims the primary native window, configures an SDR swapchain, and initializes the SDL3 and SDL_GPU Dear ImGui backends. Partial initialization unwinds in reverse order. Shutdown removes event forwarding, waits for the GPU, shuts down renderer and platform backends, persists an optional bounded layout, destroys the context, releases the window claim, and destroys the device before the window system closes.
